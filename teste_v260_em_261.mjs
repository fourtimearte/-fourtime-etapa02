...
r = await page.evaluate(async (resp) => {
  ...
  await new Promise(s => setTimeout(s, 500));
  return { n: __campo('n'), resp: __campo('resp'), razao: __campo('razao'), doc: __campo('doc'), cep: __campo('cep'),
...
// v3.262: Responsável virou a PESSOA de contato; a razão social ganhou campo próprio
checa('razão social no campo próprio', r.razao, 'PETROLEO BRASILEIRO S A PETROBRAS');
checa('Responsável fica livre para o contato', r.resp, '');
...
r = await page.evaluate(async (resp) => {
  ...
  window.__digita('cli_doc', '33000167000101');
  await new Promise(s => setTimeout(s, 500));
  return { n: __campo('n'), resp: __campo('resp'), razao: __campo('razao') };
}, CNPJ);
checa('o nome digitado fica', r.n, 'POSTO DA ESQUINA');
checa('  e a razão social vai para o campo dela', r.razao, 'PETROLEO BRASILEIRO S A PETROBRAS');
