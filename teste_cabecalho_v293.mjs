...
const ARQ = process.env.FT_ARQ || 'fourtime-editor-v293.html';
const falhas=[];
...
const grade=await p.evaluate(()=>{
  ...
  const itens=[...h.children].filter(c=>!c.classList.contains('hd-oculto')).map(c=>{
    ...
    return {rot:(c.querySelector('.hd-label')||{}).textContent||'LOGO',
...
checa('a ordem no arquivo é a mesma da tela',
  ordem, ['Cliente','CPF/CNPJ','Vendedor','Departamento','Entrega','Status','Embalagem','Pagamento','Total']);
...
console.log('CABEÇALHO v3.293: status embaixo da logo, total fechando a quarta coluna');
